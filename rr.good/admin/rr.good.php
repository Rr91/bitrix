...
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/rr.good/include.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/rr.good/prolog.php");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["backButton"]) && strlen($_POST["backButton"]) > 0) {
	...
	if (strlen($DATA_FILE_NAME) <= 0)
		$strError.= GetMessage("GOODART_ADM_IMP_NO_DATA_FILE")."<br>";
	/*if (!CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "element_edit_any_wf_status"))
		$strError.= GetMessage("GOODART_ADM_IMP_NO_IBLOCK")."<br>";*/
	if ($IBLOCK_ID=='') 
		$strError.= GetMessage("GOODART_NO_IBLOCK_ID")."<br>";
	if ($CODE_PROP=='') 
		$strError.= GetMessage("GOODART_NO_CODE_PROP")."<br>";
	if ($IBLOCK_ID!='' && $CODE_PROP!='') {
		$res = CIBlockProperty::GetByID($CODE_PROP, $IBLOCK_ID, $IBLOCK_TYPE_ID);
		if(!$ar_res = $res->GetNext())
			$strError.= GetMessage("GOODART_NO_CODE_PROP_IBLOCK")."<br>";
	}
	if(strlen($strError) <= 0) {
		//  тут парсинг xsl файла
		$res_set = CRrGood::uploadGoods($FILE_NAME);		
	}
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["backButton"]) && strlen($_POST["backButton"]) > 0 && $strError=='') {
		$str_success_detail = str_replace("#COL_INSERT#",$res_set["count_insert"],GetMessage("GOODART_ADM_EXP_LINES_EXPORTED"));
		$str_success_detail = str_replace("#COL_UPDATE#",$res_set["count_update"],$str_success_detail);
		$str_success_detail = str_replace("#COL_ERROR#",$res_set["count_false"],$str_success_detail);
		echo CAdminMessage::ShowMessage(array(
			"TYPE" => "PROGRESS",
			"MESSAGE" => GetMessage("GOODART_ADM_EXP_SUCCESS"),
...
<form method="POST" action="<?echo $sDocPath ?>?lang=<?echo LANG ?>" ENCTYPE="multipart/form-data" name="dataload" id="dataload">
	<?$aTabs = array(
		array(
			"DIV" => "edit1",
			"TAB" => GetMessage("GOODART_ADM_IMP_TAB1") ,
			"ICON" => "iblock",
			"TITLE" => GetMessage("GOODART_ADM_IMP_TAB1_ALT"),
		) ,
	);
	...
	?>
	<tr>
		<td width="40%"><?echo GetMessage("GOODART_ADM_IMP_DATA_FILE"); ?></td>
		<td width="60%">
			<input type="text" name="URL_DATA_FILE" value="<?echo htmlspecialcharsbx($URL_DATA_FILE); ?>" size="30">
			<input type="button" value="<?echo GetMessage("GOODART_ADM_IMP_OPEN"); ?>" OnClick="BtnClick()">
	...
	<tr>
		<td>&nbsp;</td>
		<td><input type="submit" name="backButton" value="<?echo GetMessage("GOODART_ADM_IMG_LOAD"); ?>" class="adm-btn-save"></td>
	</tr>
</form>
<?
$tabControl->End();
